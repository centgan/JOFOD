...
const PostingComponentTwo = ({formData, handleChange}) => {
  ...
  return (
    <div className="max-w-7xl mx-auto p-8 bg-white rounded-lg shadow-lg">
      ...
      <h3 className="text-xl font-semibold text-gray-800 mb-6">Additional questions you want to ask</h3>

      <form>
        {/* Dynamically Render Rows */}
        {/*{JSON.parse(formData.addition).map((row) => (*/}
        {rows.map((row) => (
          <div key={row.id} className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
            {/* First Input Field */}
            <div>
              <label htmlFor={`value1-${row.id}`} className="block text-sm font-medium text-gray-700">
                Question:
              </label>
              <input
                type="text"
            ...
            {/* Second Input Field */}
            <div>
              <label htmlFor={`value2-${row.id}`} className="block text-sm font-medium text-gray-700">
                Input type:
              </label>
              <select
                id={`value2-${row.id}`}
